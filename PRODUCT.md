...
Human-equivalent QA is a **charter**, not a list of `click:` steps. Example goal: `Walk booking and check a Sat-Sat week uses the weekly rate`. If `stories` is omitted and `goal`/`charter` is set, a built-in explorer (no LLM) opens the page, collects visible Book/Buy/Checkout/Cart/Setup/Rent/Continue/Add CTAs, clicks the most relevant one, then a visible cart/setup/review control when the goal mentions dates/week/cart. If date inputs appear and the goal asks for a week, it fills a Sat-Sat pair (next Saturday → Saturday+7, ISO). It records duration text and totals. It never fills payment fields and never submits Send / Submit request / Pay / Place order.

A judge then writes `human_note` + `report.findings[]` from a fixed rubric: money (duration vs selected dates, weekly rate not applied when stay ≥ 7 nights — not a ~3x day:week list price by itself), dead CTA (click did nothing / same URL / hidden-only), copy vs behavior when the goal mentions dates/pay/cart, smoke-only if nothing was clicked, and **charter incomplete** (high) when the goal mentions dates/week/price/total but the explorer never used date inputs, saw a duration string, or saw a cart total. That last case is `needs_review`, not pass. Any high-severity finding (off-by-one dates, dead CTA, weekly rate not applied, charter incomplete) is also `needs_review`, not pass. Severity is high/med/low/info. The judge does not invent bugs that were not observed.
